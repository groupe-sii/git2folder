...
class GitToFolder {
    constructor(options) {
        ...
        this.excludedBranches = ['develop', 'master'];
    }
    ...
    getDirectories() {
        return fs
            .readdirSync(this.extractFolder)
            .filter(file =>
                fs.statSync(path.join(this.extractFolder)).isDirectory()
            );
    }
    ...
    folderToBranch() {
        ...
        for (let branch of branches) {
            ...
            execSync(`mv ${this.gitFolder}/.git .git`);

            execSync(`rm -rf ${this.gitFolder}`);

            execSync(
                `cp -r ${this.extractFolder}/${branch}/. ${this.gitFolder}`
            );

            execSync(`mv .git ${this.gitFolder}/.git`);

            console.log(`commit de ${branch}`);
            execSync(
                `git add --all && git commit -m 'update ${branch}'`,
                this.gitExecOptions
            );
        }
